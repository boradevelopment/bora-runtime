...
void* os_reserve_memory(uint64_t size) {
    // Reserve address space without backing it (PROT_NONE)
    return mmap(NULL, size, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
}

bool os_commit_memory(void* addr, uint64_t size) {
    // Commit memory: set protection to readable & writable
    return mprotect(addr, size, PROT_READ | PROT_WRITE) == 0;
}

void os_free_reserved_memory(void* addr, uint64_t size) {
    munmap(addr, size);
}
...
#endif
int
uasm_allocate_dynmemory(uint8 **data, bool is_shared_memory,
                            bool is_memory64, uint64 num_bytes_per_page,
                            uint64 init_page_count, uint64 max_page_count,
                            uint64 *memory_data_size)
{
    uint64 map_size, page_size;
    uint8 *reserved_memory;
    uint64 init_size;
    ...
    // Calculate initial and max size
    init_size = init_page_count * num_bytes_per_page;
    map_size = max_page_count * num_bytes_per_page;

    // Align both sizes to page boundaries
    init_size = align_as_and_cast(init_size, page_size);
    map_size = align_as_and_cast(map_size, page_size);

    // Safety check against platform limits
    bh_assert(init_size <= GET_MAX_LINEAR_MEMORY_SIZE(is_memory64));
    bh_assert(map_size <= GET_MAX_LINEAR_MEMORY_SIZE(is_memory64));

    // === Reserve virtual address space (no physical memory yet) ===
    reserved_memory = os_reserve_memory(map_size);
    if (!reserved_memory)
        return BHT_ERROR;
    ...
    *data = reserved_memory;
    *memory_data_size = init_size;

    // Ensure 8-byte alignment for AOT safety
    bh_assert(((uintptr_t)*data & 0x7) == 0);
